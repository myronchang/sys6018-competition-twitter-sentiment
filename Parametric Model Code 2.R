...
news.tfidf = DocumentTermMatrix(news, control = list(weighting = weightTf))
news.tfidf
news.tfidf[1:5,1:5]
...
news.clean.tfidf = DocumentTermMatrix(news.clean, control = list(weighting = weightTf))
news.clean.tfidf
...
# Linear regression (all 43 variables)
...
# Linear regression #2 (only variables with significance markers '.'s or '*'s from first model)
...
cvv <- sapply(X = 1:nrow(newdata2), function(X){
  ...
  guess <- round(predict(cvmodel, newdata=testing_data))
...
# Linear regression #3 (only variables with *s from model 2)
...
# Linear regression #4 (only variables with ** and *** from model 3)
...
test.tfidf = DocumentTermMatrix(test, control = list(weighting = weightTf))
test.tfidf
test.tfidf[1:5,1:5]
...
# recompute TF-IDF matrix
test.clean.tfidf = DocumentTermMatrix(test.clean, control = list(weighting = weightTf))
test.clean.tfidf
