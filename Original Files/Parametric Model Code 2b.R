...
# Histogram of sentiments (for data exploration)
...
# 26 terms
tfidf.97 = removeSparseTerms(news.clean.tfidf, 0.97)  # remove terms that are absent from at least 99% of documents (keep most terms)
tfidf.97
as.matrix(tfidf.97[1:5,1:5])
as.matrix(tfidf.97)
#as.data.frame(tfidf.97)

# 16 terms
tfidf.96 = removeSparseTerms(news.clean.tfidf, 0.96)  # remove terms that are absent from at least 99% of documents (keep most terms)
tfidf.96
as.matrix(tfidf.96[1:5,1:5])
as.matrix(tfidf.96)
#as.data.frame(tfidf.96)

# 11 terms
tfidf.95 = removeSparseTerms(news.clean.tfidf, 0.95)  # remove terms that are absent from at least 99% of documents (keep most terms)
tfidf.95
as.matrix(tfidf.95[1:5,1:5])
as.matrix(tfidf.95)
#as.data.frame(tfidf.95)

# 6 terms
tfidf.94 = removeSparseTerms(news.clean.tfidf, 0.94)  # remove terms that are absent from at least 99% of documents (keep most terms)
tfidf.94
as.matrix(tfidf.94[1:5,1:5])
as.matrix(tfidf.94)
#as.data.frame(tfidf.94)
cleaneddata <- as.data.frame(as.matrix(tfidf.94))
newdata <- cbind(data,cleaneddata)
...
# Model GOOGLE
google <- glm(sentiment~googl, data=newdata2)
summary(google)
anova(google)
cv.err.google = cv.glm(newdata2, google)
cv.err.google$delta
...
predictions <- as.data.frame(round(predict(model, newdata=newtestdata)))
names(predictions) <- "sentiment"
submission <- cbind(newtestdata$id, predictions)
names(submission) <- c("id","sentiment")

predictions <- as.data.frame(round(predict(google, newdata=newtestdata)))
names(predictions) <- "sentiment"
submission <- cbind(newtestdata$id, predictions)
names(submission) <- c("id","sentiment")
